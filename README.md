...
- Three accounts to segregate accounts by function.
  ...
  - automation account: meshStack will use this account to invoke the Lambda functions that are used in [Landing Zones](https://docs.meshcloud.io/docs/meshcloud.landing-zones.html).
...
| <a name="input_aws_sso_instance_arn"></a> [aws\_sso\_instance\_arn](#input\_aws\_sso\_instance\_arn) | ARN of the AWS SSO instance to use. When set, meshStack replicates IAM permissions for managed AWS Accounts to AWS IAM Identity Center. Set to null to disable IAM permission replication. Setup instructions at https://docs.meshcloud.io/docs/meshstack.aws.sso-setup.html. | `string` | `null` | no |
...
| <a name="input_create_access_keys"></a> [create\_access\_keys](#input\_create\_access\_keys) | Set to true to enable creation of service account access keys. This is not recommended for production environments. Consider using workload identity federation instead by setting workload\_identity\_federation variable. | `bool` | `false` | no |
